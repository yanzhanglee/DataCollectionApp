...
//    public class MyAccessibilityServiceInner extends AccessibilityService {
...
/*        if(nextPckg.contains(Constants.WeChatPackageName)){
            return;
        }*/
        // End timeout when app is switched (does not end when keyboard is pulled out but that should never happen)
//        if (timeoutOn){
//            Log.d(TAG, "~~~Timeout sending START INTENT because of a new event");
//            Log.d(TAG, nextPckg);
//            Log.d(TAG, "~~~"+(String) currAppData.get("package_name"));
//            Intent startIntent = new Intent(Plugin.this, Timeout.class);
//            startIntent.setAction(Constants.ACTION_START_APPLICATION).putExtra("phase",currentPhase);
//            Log.d(TAG, "administerIntervention: Timeout.class start 0");
    ...
    protected void attachAppListener() {
        ...
        Applications.setSensorObserver(new Applications.AWARESensorObserver() {
            ...
            public void onForeground(ContentValues data) {
                ...
                /*if (currAppData.size() > 0 && !nextPckg.equals((String) currAppData.get("package_name"))){
                    Constants.lastUsingTime.put(currAppPackageName, System.currentTimeMillis());
                    Log.d(TAG, "SWITCH to "+nextPckg);
                }*/
                /*if(Constants.enterAppCounter.get(currAppPackageName) == null){
                    Constants.lastUsingTime.put(currAppPackageName, System.currentTimeMillis());
...
//                    }
                ...
                if(whitelistedApps.contains(package_name)) {
                    ...
                    if(package_name.equals(Constants.WeChatPackageName)) {
                        ...
                        found_flag = false;
                        /*Log.d(TAG, "***onForeground: currentApp = "+currentComponentName.flattenToShortString());
                        ...
                        for(String bad_activity:WeChatActivities) {
                            if(bad_activity.equals(currentComponentName.flattenToShortString())) {
                                if(Constants.lastInputRight.get(package_name) == null){
                                    Log.d(TAG, "SNS NULL ");
                                    found_flag = true;
                                    break;
                                }else if(Constants.lastInputRight.get(package_name)){
                                    Log.d(TAG, "SNS FALSE? " + Constants.lastInputRight.get(package_name));
                                    if(((System.currentTimeMillis() - Constants.lastSNSTime[0]) > SNSInterval)){
                                        found_flag = true;
                                        break;
                                    }
                                }else {
                                    Constants.lastInputRight.get(package_name);
                                }
                            }
                        }
                        Log.d(TAG, "***onForeground: found_flag = "+found_flag);*/
                        /*Log.d(TAG, "WECHAT LAST:  " + Constants.lastWechatEnterTime[0] +
                                " WECHAT NEW: " + System.currentTimeMillis() + " DIFF : " +
...
//        Log.v(TAG, DatabaseUtils.dumpCursorToString(prevAppDataCursor));
    ...
    private void waitingPopUp(){
            ...
            /*Log.d(TAG, "SNS: " + currAppPackageName + " " + Constants.WeChatPackageName + " " +
                            (currAppPackageName.equals(Constants.WeChatPackageName)) + " " +
                            currentComponentName + " LAST RIGHT: " +
                            Constants.lastInputRight.get(currAppPackageName) +
                    " FOR: " + (System.currentTimeMillis() - Constants.lastSNSTime[0]));*/
